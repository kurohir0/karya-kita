                    ...
                    <table class="min-w-full mt-4 border border-gray-300">
                        <thead>
                            <tr>
                                ...
                                <th class="border border-gray-300 px-4 py-2">Pengupload</th>
                        ...
                        <tbody>
                            @foreach ($karyaList as $karya)
                                <tr>
                                    ...
                                    <td class="border border-gray-300 px-4 py-2">{{ $karya->kategori }}</td>
                                    <td class="border border-gray-300 px-4 py-2">{{ $karya->user->name }}</td>
                                    <td class="border border-gray-300 px-4 py-2">
                                        {{ $karya->is_confirmed ? 'Terkonfirmasi' : 'Belum Terkonfirmasi' }}</td>
                                    <td class="border border-gray-300 px-4 py-2">
                                        <a href="{{ route('admin.karya.show', $karya->id) }}"
                                            class="text-blue-500">Detail</a>
                                        <form action="{{ route('admin.karya.destroy', $karya->id) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            @method('DELETE')
